...
final class RY_IFSMILEPAY_WC_Invoice
{
    ...
    protected function do_init(): void
    {
        ...
        add_action('ry_invoice_smilepay-post_get_invoice', [$this, 'do_get_invoice'], 0, 3);
    ...
    public function do_get_invoice($post_args, $result, $order_ID)
    {
        ...
        if (apply_filters('ry_invoice-wc_success_notice', true)) {
            $order->add_order_note(
                __('Invoice number', 'ry-invoice-for-smilepay') . ': ' . (string) $result->InvoiceNumber . "\n"
                . __('Invoice random number', 'ry-invoice-for-smilepay') . ': ' . (string) $result->RandomNumber . "\n"
                . __('Invoice create time', 'ry-invoice-for-smilepay') . ': ' . $invoice_date->format('Y-m-d H:i:s'),
            );
        }
    ...
    public function do_delete_invoice($post_args, $result, $order_ID)
    {
        ...
        if ((string) $result->Status != '0') {
            $order->add_order_note(sprintf(
                /* translators: %1$s Error messade, %2$s Status code */
                __('Invalid invoice error: %1$s (%2$s)', 'ry-invoice-for-smilepay'),
                (string) $result->Desc,
                (string) $result->Status,
            ));
            return;
        ...
        if (apply_filters('ry_invoice-wc_success_notice', true)) {
            $order->add_order_note(
                __('Invalid invoice', 'ry-invoice-for-smilepay') . ': ' . $order->get_meta('_invoice_number'),
            );
        }
    ...
    public function get_invoice($order)
    {
        ...
        if ($invoice_data['total'] == 0) {
            ...
            $order->save();
            $order->add_order_note(__('Zero total fee without invoice', 'ry-invoice-for-smilepay'));
            return;
        }
        if ($invoice_data['total'] < 0) {
            $order->update_meta_data('_invoice_number', 'negative');
            $order->save();
            $order->add_order_note(__('Negative total fee can\'t invoice', 'ry-invoice-for-smilepay'));
            return;
        }
        ...
        RY_Logs::log('smilepay-invoice', 'info', 'Get WooCommerce #' . $order->get_id(), $invoice_data);
    ...
    public function invalid_invoice($order)
    {
        ...
        RY_Logs::log('smilepay-invoice', 'info', 'Invalid WooCommerce #' . $order->get_id(), $invoice_data);
