    ...
    protected static function init_fields($order)
    {
        self::$fields = [
            ...
            'carruer_type' => [
                ...
                'type' => 'select',
                'options' => [
                    'smilepay_host' => rywai_carruer_type_to_name('smilepay_host'),
        ...
        if (!isset(self::$fields['carruer_type']['options'][$invoice_type])) {
            self::$fields['carruer_type']['options'][$invoice_type] = rywai_carruer_type_to_name($invoice_type);
        }
...
<div class="ivoice <?php echo($invoice_number ? '' : 'address'); ?>">
    <div class="ivoice_data_column">
        <p>
            ...
            <strong><?php esc_html_e('Invoice type', 'ry-invoice-for-smilepay'); ?>:</strong> <?php echo esc_html(rywai_invoice_type_to_name($invoice_type)); ?><br>

            <?php if ($invoice_type === 'personal') { ?>
            <strong><?php esc_html_e('Carruer type', 'ry-invoice-for-smilepay'); ?>:</strong> <?php echo esc_html(rywai_carruer_type_to_name($carruer_type)); ?><br>
