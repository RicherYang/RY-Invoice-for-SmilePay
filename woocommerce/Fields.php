...
final class Fields
{
    ...
    {
        $asset_info = include RY_IFSMILEPAY_PLUGIN_DIR . 'assets/wc-checkout.asset.php';
        wp_enqueue_script('ry-invoice-wc-checkout', RY_IFSMILEPAY_PLUGIN_URL . 'assets/wc-checkout.js', $asset_info['dependencies'], $asset_info['version'], true);

        $args = [
            'checkout' => $checkout,
        ];
        wc_get_template('checkout/form-invoice.php', $args, '', RY_IFSMILEPAY_PLUGIN_DIR . 'woocommerce/templates/');
    }
                    ...
                    case 'MOICA':// 自然人憑證
                        if (!empty($data['invoice_carruer_no'])) {
                            if (!preg_match('/^[A-Z]{2}\d{14}$/', $data['invoice_carruer_no'])) {
                                $errors->add('validation', __('Invalid carruer number', 'ry-invoice-for-smilepay'));
                            }
                        }
                        break;
            ...
            case 'donate': // 愛心碼
                if (!empty($data['invoice_donate_no'])) {
                    if (!preg_match('/^[0-9]{3,7}$/', $data['invoice_donate_no'])) {
                        $errors->add('validation', __('Invalid donate number', 'ry-invoice-for-smilepay'));
                    }
                }
                break;
            case 'company':// 公司
                if (!empty($data['invoice_no'])) {
                    ...
                    if (!$valid) {
                        $errors->add('validation', __('Invalid tax ID number', 'ry-invoice-for-smilepay'));
                    }
                }
                break;
        }
    ...
    public function show_invoice_info($order)
    {
        ...
        if ($invoice_number) {
            if (preg_match('/^[A-Z]{2}[0-9]{8}$/', $invoice_number)) {
                $invoice_info[] = [
                    'key' => 'invoice-number',
                    'name' => __('Invoice number', 'ry-invoice-for-smilepay'),
                    'value' => $invoice_number,
                ];
                $invoice_info[] = [
                    'key' => 'invoice-random-number',
                    'name' => __('Invoice random number', 'ry-invoice-for-smilepay'),
                    'value' => $order->get_meta('_invoice_random_number'),
                ];
                $invoice_info[] = [
                    'key' => 'invoice-date',
                    'name' => __('Invoice date', 'ry-invoice-for-smilepay'),
                ...
                switch ($invoice_number) {
                    ...
                    case 'zero':
                        $invoice_info[] = [
                            'key' => 'zero-info',
                            'name' => __('Zero total fee without invoice', 'ry-invoice-for-smilepay'),
                            'value' => '',
                        ];
                        break;
                    case 'negative':
                        $invoice_info[] = [
                            'key' => 'negative-info',
                            'name' => __('Negative total fee can\'t invoice', 'ry-invoice-for-smilepay'),
                            'value' => '',
                        ];
                        break;
                }
        ...
        switch ($invoice_type) {
            case 'personal':
                ...
                if (in_array($carruer_type, ['MOICA', 'phone_barcode'])) {
                    $invoice_info[] = [
                        'key' => 'carruer-number',
                        'name' => __('Carruer number', 'ry-invoice-for-smilepay'),
            ...
            case 'company':
                $invoice_info[] = [
                    'key' => 'tax-id-number',
                    'name' => __('Tax ID number', 'ry-invoice-for-smilepay'),
            ...
            case 'donate':
                $invoice_info[] = [
                    'key' => 'donate-number',
                    'name' => __('Donate number', 'ry-invoice-for-smilepay'),
        ...
        wc_get_template('order/order-invoice-info.php', $args, '', RY_IFSMILEPAY_PLUGIN_DIR . 'woocommerce/templates/');
    }
}
