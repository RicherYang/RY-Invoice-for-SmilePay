...
abstract class AbstractAdminOrder
{
    ...
    public function bulk_action_notices()
    {
        ...
        if ($bulk_action === 'ry_get_invoice') {
            ...
            /* translators: %s: count */
            $message = sprintf(_n('%s order issue invoice.', '%s orders issue invoice.', $number, 'ry-invoice-for-smilepay'), number_format_i18n($number));
    ...
    public function add_invoice_column($columns)
    {
        if (!isset($columns['invoice-number'])) {
            $add_columns = [
                'invoice-number' => __('Invoice number', 'ry-invoice-for-smilepay'),
            ];
        ...
        if ($column === 'invoice-number') {
            ...
            if (!empty($invoice_number)) {
                match ($invoice_number) {
                    'wait' => esc_html_e('Wait get invoice', 'ry-invoice-for-smilepay'),
                    'zero' => esc_html_e('Zero no invoice', 'ry-invoice-for-smilepay'),
                    'negative' => esc_html_e('Negative no invoice', 'ry-invoice-for-smilepay'),
    ...
    {
        $actions['ry_get_invoice'] = __('Issue invoice', 'ry-invoice-for-smilepay');
...
<div class="ivoice <?php echo($invoice_number ? '' : 'address'); ?>">
    <div class="ivoice_data_column">
        <p>
            ...
            <strong><?php esc_html_e('Expected get time', 'ry-invoice-for-smilepay'); ?>:</strong> <?php echo esc_html($scheduled_time->format('Y-m-d H:i')); ?><br>
            <?php } ?>
            <?php
                    break;
                case 'zero': ?>
            ...
            <strong><?php esc_html_e('Carrier type', 'ry-invoice-for-smilepay'); ?>:</strong> <?php echo esc_html(Utils::carruer_type_to_name($carruer_type)); ?><br>

            <?php if (in_array($carruer_type, ['MOICA', 'phone_barcode'])) { ?>
            <strong><?php esc_html_e('Carrier number', 'ry-invoice-for-smilepay'); ?>:</strong> <?php echo esc_html($order->get_meta('_invoice_carruer_no')); ?><br>
            <?php } ?>
            <?php } ?>
