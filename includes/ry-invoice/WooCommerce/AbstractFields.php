...
abstract class AbstractFields
{
    ...
    abstract protected function is_move_billing_company(): bool;
    ...
    public function add_invoice_info($fields)
    {
        ...
        $fields['invoice'] = [
            ...
            'invoice_carruer_type' => [
                'type' => 'select',
                'label' => __('Carrier type', 'ry-invoice-for-smilepay'),
                'options' => [
                    $host_type => Utils::carruer_type_to_name($host_type) . __(' (send paper when win)', 'ry-invoice-for-smilepay'),
            ...
            'invoice_carruer_no' => [
                'label' => __('Carrier number', 'ry-invoice-for-smilepay'),
                'required' => true,
                'priority' => 20,
            ],
            'invoice_no' => [
        ...
        if ($this->is_move_billing_company()) {
    ...
    protected function get_invoice_info($order)
    {
        ...
        switch ($invoice_type) {
            case 'personal':
                ...
                if (in_array($carruer_type, ['MOICA', 'phone_barcode'])) {
                    $invoice_info[] = [
                        'key' => 'carruer-number',
                        'name' => __('Carrier number', 'ry-invoice-for-smilepay'),
